...
fn get_joint_type(node_1: *struc CExp, node_2: *struc CExp) *struc Type {
    ...
    elif node_1[].exp_type[].tag == 8 or node_2[].exp_type[].tag == 8 {
...
fn get_joint_ptr_type(ctx: *struc SemanticContext, node_1: *struc CExp, node_2: *struc CExp, joint_type: **struc Type) i32 {
    ...
    elif node_1[].tag == 92 and is_const_null_ptr(@node_1[].get._CConstant) {
        if node_2[].exp_type ~= joint_type[] {
            free_Type(@joint_type[])
            joint_type[] = node_2[].exp_type
            (joint_type[])[]._ref_count++
    ...
    elif (node_2[].tag == 92 and is_const_null_ptr(@node_2[].get._CConstant)) or (node_1[].exp_type[].tag == 11 and node_1[].exp_type[].get._Pointer.ref_type[].tag == 9 and node_2[].exp_type[].tag == 11) {
        if node_1[].exp_type ~= joint_type[] {
            free_Type(@joint_type[])
            joint_type[] = node_1[].exp_type
            (joint_type[])[]._ref_count++
    ...
    elif node_2[].exp_type[].tag == 11 and node_2[].exp_type[].get._Pointer.ref_type[].tag == 9 and node_1[].exp_type[].tag == 11 {
        if node_2[].exp_type ~= joint_type[] {
            free_Type(@joint_type[])
            joint_type[] = node_2[].exp_type
            (joint_type[])[]._ref_count++
...
fn get_compound_info_at(node: *struc CCompoundInit) u64 {
    ...
    loop while initializer[].tag == 137 {
...
fn cast_assign(ctx: *struc SemanticContext, exp_type: **struc Type, exp: **struc CExp) i32 {
    ...
    type_fmt_2: string = ? 0 then sdsnew(0) else 0
    _errval: i32 = 0
    if (is_type_arithmetic((exp[])[].exp_type) and is_type_arithmetic(exp_type[])) or ((exp[])[].tag == 92 and (exp_type[])[].tag == 11 and is_const_null_ptr(@(exp[])[].get._CConstant)) or ((exp_type[])[].tag == 11 and (exp_type[])[].get._Pointer.ref_type[].tag == 9 and (exp[])[].exp_type[].tag == 11) or ((exp[])[].exp_type[].tag == 11 and (exp[])[].exp_type[].get._Pointer.ref_type[].tag == 9 and (exp_type[])[].tag == 11) {
        loop .. while 0 {
...
fn check_binary_add_exp(ctx: *struc SemanticContext, node: *struc CBinary) i32 {
    ...
    elif node[].exp_left[].exp_type[].tag == 11 and is_type_complete(ctx, node[].exp_left[].exp_type[].get._Pointer.ref_type) and is_type_int(node[].exp_right[].exp_type) {
    ...
    elif is_type_int(node[].exp_left[].exp_type) and node[].exp_right[].exp_type[].tag == 11 and is_type_complete(ctx, node[].exp_right[].exp_type[].get._Pointer.ref_type) {
        common_type = make_Long()
...
fn check_binary_subtract_exp(ctx: *struc SemanticContext, node: *struc CBinary) i32 {
    ...
    if is_type_arithmetic(node[].exp_left[].exp_type) and is_type_arithmetic(node[].exp_right[].exp_type) {
    ...
    elif node[].exp_left[].exp_type[].tag == 11 and is_type_complete(ctx, node[].exp_left[].exp_type[].get._Pointer.ref_type) {
        ...
        elif is_same_type(node[].exp_left[].exp_type, node[].exp_right[].exp_type) and not (node[].exp_left[].tag == 92 and is_const_null_ptr(@node[].exp_left[].get._CConstant)) {
            common_type = make_Long()
            if common_type ~= node[]._base[].exp_type {
                free_Type(@node[]._base[].exp_type)
                node[]._base[].exp_type = common_type
...
fn check_conditional_exp(ctx: *struc SemanticContext, node: *struc CConditional) i32 {
    ...
    elif node[].exp_middle[].exp_type[].tag == 9 and node[].exp_right[].exp_type[].tag == 9 {
    ...
    elif node[].exp_middle[].exp_type[].tag == 11 or node[].exp_right[].exp_type[].tag == 11 {
        loop .. while 0 {
...
fn check_subscript_exp(ctx: *struc SemanticContext, node: *struc CSubscript) i32 {
    ...
    _errval: i32 = 0
    if node[].primary_exp[].exp_type[].tag == 11 and is_type_complete(ctx, node[].primary_exp[].exp_type[].get._Pointer.ref_type) and is_type_int(node[].subscript_exp[].exp_type) {
    ...
    elif is_type_int(node[].primary_exp[].exp_type) and node[].subscript_exp[].exp_type[].tag == 11 and is_type_complete(ctx, node[].subscript_exp[].exp_type[].get._Pointer.ref_type) {
        subscript_type = make_Long()
...
fn check_fun_params_decl(ctx: *struc SemanticContext, node: *struc CFunctionDeclaration) i32 {
    ...
    loop i: u64 = 0 while i < (? (node[].params) then (cast<*struc stbds_array_header>((node[].params)) - 1)[].length else 0) .. ++i {
        ...
        if fun_type[].param_types[i][].tag == 12 {
...
fn check_fun_decl(ctx: *struc SemanticContext, node: *struc CFunctionDeclaration) i32 {
    ...
    is_glob: i32 = node[].storage_class.tag ~= 133
...
fn push_zero_static_init(ctx: *struc SemanticContext, byte: i64) none {
    if not ((? (ctx[].p_static_inits[]) then (cast<*struc stbds_array_header>((ctx[].p_static_inits[])) - 1)[].length else 0) == 0) and (ctx[].p_static_inits[])[(? (ctx[].p_static_inits[]) then (cast<*struc stbds_array_header>((ctx[].p_static_inits[])) - 1)[].length else 0) - 1][].tag == 22 {
...
fn check_file_var_decl(ctx: *struc SemanticContext, node: *struc CVariableDeclaration) i32 {
    ...
    is_glob = node[].storage_class.tag ~= 133
    if node[].init {
...
fn reslv_block_var_decl(ctx: *struc SemanticContext, node: *struc CVariableDeclaration) i32 {
    ...
    elif node[].storage_class.tag == 134 {
        loop .. while 0 {
    ...
    if node[].init and node[].storage_class.tag == 132 {
        loop .. while 0 {
