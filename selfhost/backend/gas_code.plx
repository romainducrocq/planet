...
fn data_op(ctx: *struc GasCodeContext, node: *struc AsmData) none {
    ...
    if map_it ~= -1 {
        ...
        if backend_obj_symbol[].tag == 44 and backend_obj_symbol[].get._BackendObj.is_const {
            emit(ctx, ".L")
        }
    }
...
fn binary_instr(ctx: *struc GasCodeContext, node: *struc AsmBinary) none {
    emit(ctx, "    "         "    ")
    {
        is_dbl: i32 = node[].asm_type[].tag == 41
        emit(ctx, get_binop(@node[].binop, is_dbl))
        is_packed: i32 = node[].binop.tag == 264 and is_dbl
...
fn static_section_toplvl(ctx: *struc GasCodeContext, node_list: **struc StaticInit) none {
    if (? (node_list) then (cast<*struc stbds_array_header>((node_list)) - 1)[].length else 0) == 1 and node_list[0][].tag == 22 {
        emit(ctx, "    "             ".bss"             "\n")
    }
    else {
        emit(ctx, "    "             ".data"             "\n")
