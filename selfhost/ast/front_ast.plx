...
pub fn make_CIf(condition: **struc CExp, then: **struc CStatement, else_fi: **struc CStatement) *struc CStatement {
    ...
    ;
    self[].get._CIf.then = 0
    if then[] ~= self[].get._CIf.then {
        free_CStatement(@self[].get._CIf.then)
        self[].get._CIf.then = then[]
        then[] = 0
    }
    ;
...
pub fn make_CSwitch(match: **struc CExp, body: **struc CStatement) *struc CStatement {
    ...
    self[].get._CSwitch.match = 0
    if match[] ~= self[].get._CSwitch.match {
        free_CExp(@self[].get._CSwitch.match)
        self[].get._CSwitch.match = match[]
        match[] = 0
    }
    ;
