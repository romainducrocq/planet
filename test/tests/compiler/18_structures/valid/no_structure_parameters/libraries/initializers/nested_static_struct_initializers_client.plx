#  Test initialization of nested static structs, including:
#  * - partial initialization
#  * - arrays of structs, structs containing arrays
#  * - implicit conversion of scalar elements, array decay of string literals
#  

import "nested_static_struct_initializers"













#  structs defined here
#  validation functions defined in library

#  case 1: struct with no explicit initializer should be all zeros
pub all_zeros: struc outer;

#  case 2: partially initialized struct
pub partial: struc outer = $(    100l,     $(10, $(10)), #  leave arr[1], arr[2], and y uninitialized
    "Hello!") #  leave d uninitialized

pub full: struc outer = $(    18014398509481979l,     $(1000, "ok",     4292870144u), #  can initialized signed char array w/ static string
    "Another message",     2e12)

pub converted: struc outer = $(    10.5, #  10l
    $(    2147483650u, #  -2147483646
    $(    15.6, #  15
    17592186044419l, #  3
    2147483777u#  -127
    ),     1152921506754330624ul#  2147483648u
    ),     0ul, #  null pointer
    9223372036854776833ul#  9223372036854777856.0
    )

pub struct_array: [3]struc outer = $($(1, $(2, "ab", 3), 0, 5),     $(6, $(7, "cd", 8), "Message", 9))

pub fn main(none) i32 {
    if not test_uninitialized() {
        return 1
    }

    if not test_partially_initialized() {
        return 2
    }

    if not test_fully_intialized() {
        return 3
    }

    if not test_implicit_conversions() {
        return 4
    }

    if not test_array_of_structs() {
        return 5
    }

    return 0 #  success
}
