...
pub fn target_from_int(none) f64 {
    return cast<f64>(1000)
}

#  cast a double outside the range of int
#  (See example in Chapter 13, "Converting an Unsigned Integer to a double")
pub fn target_from_uint(none) f64 {
    return cast<f64>(4294967290u)
}
...
#  to a double")
pub fn target_from_long(none) f64 {
...
#  (From double-rounding example in Chapter 13, "Converting an Unsigned Integer
#  to a double")
pub fn target_from_ulong(none) f64 {
    return cast<f64>(9223372036854776833ul)
}

#  same as target_from_int but cast is implicit;
#  make sure we still constant fold it
pub fn target_implicit(none) f64 {
    return 1000
}
